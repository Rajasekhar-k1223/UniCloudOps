...
import { Plus, Trash2, ShieldCheck, X, RefreshCw, Layers, Play, Pause } from 'lucide-react';
...
const CloudAccounts = () => {
  ...
  const handleToggle = async (id) => {
    try {
      await api.post(`/cloud-accounts/${id}/toggle`);
      fetchAccounts();
    } catch (err) {
      console.error("Status toggle failed", err);
      alert("Failed to toggle status: " + (err.response?.data?.detail || "Network Error"));
    }
  };
            ...
            <div key={acc.id} className="bg-white border border-slate-200 p-8 relative rounded-[2rem] shadow-sm hover:shadow-xl transition-all duration-500 group overflow-hidden">
              ...
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  <div className={`w-3 h-3 rounded-full ${!acc.is_active ? 'bg-slate-300 shadow-[0_0_12px_rgba(148,163,184,0.4)]' :
                    ...
                    title={!acc.is_active ? "Sync Suspended" : acc.status === 'error' ? acc.error_message : acc.status}
                  />
                  <div className="flex flex-col">
                    <span className="text-[10px] font-black text-slate-800 uppercase tracking-widest">
                      {acc.provider}
                    </span>
                    <span className="text-[8px] font-bold text-slate-400 uppercase tracking-[0.2em]">{!acc.is_active ? 'PAUSED' : acc.status}</span>
                  </div>
                </div>
                <div className="flex items-center gap-2 relative z-20">
                  <button
                    onClick={() => handleToggle(acc.id)}
                    title={acc.is_active ? "Pause Sync" : "Resume Sync"}
                    className={clsx(
                      "p-2.5 rounded-xl transition-all border",
                      acc.is_active ? "bg-slate-50 text-slate-400 border-slate-100 hover:text-amber-600 hover:bg-amber-50 hover:border-amber-100" : "bg-emerald-50 text-emerald-600 border-emerald-100 hover:bg-emerald-100"
                    )}
                  >
                    {acc.is_active ? <Pause className="w-3.5 h-3.5" /> : <Play className="w-3.5 h-3.5" />}
                  </button>
                  <button
                    onClick={() => handleSync(acc.id)}
                    disabled={!acc.is_active}
                    title={!acc.is_active ? "Sync disabled (Paused)" : "Force Sync"}
                    className={clsx(
                      "p-2.5 rounded-xl transition-all border",
                      !acc.is_active ? "bg-slate-100 text-slate-300 border-slate-200 cursor-not-allowed" :
                ...
                {acc.connectivity && (
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center text-[10px] font-bold uppercase tracking-widest gap-2">
                      <ShieldCheck size={14} className={acc.connectivity.authenticated && acc.connectivity.access !== false ? "text-emerald-500" : "text-rose-500"} />
                      {acc.connectivity.authenticated ? (acc.connectivity.access !== false ? "Authenticated & Active" : "Authenticated (API Blocked)") : "Auth Failure"}
                    </div>
                    <div className="flex items-center text-[10px] font-bold uppercase tracking-widest gap-2">
                      ...
                      {acc.connectivity.billing_access ? "Fiscal Access: ACTIVE" : "Fiscal Access: BLOCKED / NONE"}
                    </div>
                    ...
                    {acc.connectivity.error && (
                      <div className="mt-2 p-3.5 bg-rose-50 border border-rose-100 rounded-2xl text-[9px] font-mono text-rose-600 leading-normal break-words">
                        <span className="font-black uppercase tracking-wider block mb-1 text-[8px] text-rose-500">Connection Cutoff / Issue Details:</span>
                        {acc.connectivity.error}
                      </div>
                    )}
                  </div>
